...
export function getLiquidationPrice(valEth, valDai) {
  ...
  const retVal = calcNewBorrowLimitAndLiquidationPrice(
    originalUserInfo,
    web3.utils.toWei(valEth.toString()),
    web3.utils.toWei(valDai.toString()),
    web3
  );
...
export function validateDeposit(val) {
  return verifyDepositInput(
    originalUserInfo,
    web3.utils.toWei(val.toString()),
    web3
  );
}
export function validateWithdraw(val) {
  return verifyWithdrawInput(
    originalUserInfo,
    web3.utils.toWei(val.toString()),
    web3
  );
...
export async function migrateMakerDao() {
  if (userInfo.bCdpInfo.hasCdp) {
    return await ApiAction(
      B.migrateToExisting(
        web3,
        userInfo.proxyInfo.userProxy,
        userInfo.makerdaoCdpInfo.cdp,
        userInfo.bCdpInfo.cdp
      ),
      user,
      web3,
      0
...
export async function deposit(amountEth, onHash) {
  ...
  if (userInfo.bCdpInfo.hasCdp) {
    return await ApiAction(
      B.depositETH(web3, userInfo.proxyInfo.userProxy, userInfo.bCdpInfo.cdp),
      user,
      web3,
      val,
      onHash
...
export async function unlock(onHash) {
  return await ApiAction(
    B.unlockDai(web3, userInfo.proxyInfo.userProxy),
    user,
    web3,
    0,
    onHash
...
export async function repay(amountDai, onHash) {
  ...
  if (Number(userInfo.bCdpInfo.daiDebt) <= Number(amountDai) + 1) {
    return await ApiAction(
      B.repayAllDai(web3, userInfo.proxyInfo.userProxy, userInfo.bCdpInfo.cdp),
      user,
      web3,
      0,
      onHash
