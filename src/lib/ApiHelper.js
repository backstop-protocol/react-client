...
const humanizeExcludeFields = ["userProxy", "daiAllowance"];
      ...
      if (isNaN(k * 1)) {
        if (result[k] instanceof Array) {
          ...
          if (
            ...
            !isNaN(result[k] * 1) &&
            humanizeExcludeFields.indexOf(k) === -1
          ) {
            res[k] =
              result[k].length > 16
                ? web3.utils.fromWei(result[k]) * 1
                : result[k] * 1;
          } else {
            res[k] = result[k];
          }
...
export function repayUnlocked(web3, userInfo) {
  return (
    web3.utils.toBN(userInfo.userWalletInfo.daiAllowance).toString(16) ===
...
export const ApiAction = async function (
  action,
  user,
  web3,
  gasValue = 0,
