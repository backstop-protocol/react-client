...
export default class EtheriumBox extends Component {
  formatValue(userInfo) {
    return userInfo ? numm(userInfo.bCdpInfo.ethDeposit, 4) : 0;
  }
  calculateUSD(userInfo) {
    return userInfo
      ? numm(userInfo.bCdpInfo.ethDeposit * userInfo.miscInfo.spotPrice, 2)
      : 0;
  }
  borrowLimit(userInfo, value) {
    return userInfo
      ? numm((userInfo.bCdpInfo.daiDebt / value) * 100, 2, 100)
      : 0;
  }
  ...
  render() {
    ...
    return (
      <CurrencyBox
        ...
        currency={"ETH"}
        icon={Etherium}
        currencyValue={userInfo ? userInfo.bCdpInfo.ethDeposit : 0}
