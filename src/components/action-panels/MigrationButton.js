...
export default class MigrationButton extends Component {
  ...
  render() {
    ...
    let extraClass =
      status !== MigrationStatus.none ? "currency-action-panel" : "";
    let btnContainerCls = status !== MigrationStatus.none ? "disabled" : "";

    return (
      <div
        className={`migrate-btn ${btnContainerCls}`}
        onClick={this.showMigratePopup}
      >
        <div className={`centered ${extraClass}`}>
          <h3>
            {icon && <img className="result" src={icon} alt="_icon" />}
